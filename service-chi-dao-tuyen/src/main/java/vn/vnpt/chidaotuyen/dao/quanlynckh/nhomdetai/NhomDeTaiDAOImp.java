...
public class NhomDeTaiDAOImp implements NhomDeTaiDAO {
    ...
    public List getDanhSachNhomDeTai() {
        ...
        for (int i = 0; i < t.size(); i++) {
            ...
            nhomDeTaiObj.trangThai = Integer.parseInt(map.get("trang_thai").toString());
    ...
    public List getDanhSachNhomDeTaiTheoTen(String nhomDeTai) {
        String sql = "CALL LayDanhSachNhomDeTaiTheoTen(?)";
        JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
        List t = jdbcTemplate.queryForList(sql, new Object[]{nhomDeTai});
        ArrayList nhomDeTaiObjList = new ArrayList();
        for (int i = 0; i < t.size(); i++) {
            Object newtmp = t.get(i);
            LinkedCaseInsensitiveMap map = (LinkedCaseInsensitiveMap) newtmp;
    ...
    public NhomDeTaiObj getNhomDeTai(Long maNhomDeTai) {
        String sql = "CALL LayNhomDeTai(?)";
        JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
        List t = jdbcTemplate.queryForList(sql, new Object[]{ maNhomDeTai });
        try
        {
            t.get(0);
        }
