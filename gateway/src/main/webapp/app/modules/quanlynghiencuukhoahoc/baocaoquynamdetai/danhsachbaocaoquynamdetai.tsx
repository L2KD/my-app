...
import { getDanhSachBaoCaoQuyNamDeTai, getDanhSachBaoCaoQuyNamDeTaiTheoTen_DonVi_Nam_TrangThai,
    getDanhSachDonVi, getDanhSachTinhThanh, resetall} from './baocaoquynamdetai.reducer';
import { IRootState } from 'app/shared/reducers';
import { IBaoCaoQuyNamDeTai } from 'app/modules/quanlynghiencuukhoahoc/baocaoquynamdetai/baocaoquynamdetai.model';

export interface IHienThiDanhSachBaoCaoQuyNamDeTaiProps extends StateProps, DispatchProps, RouteComponentProps<{
    tinhthanh: string, donvi: string, nam: string, trangthai: string }> {}

export interface IBaoCaoQuyNamDeTaiState extends IPaginationBaseState {
    ma_tinh_thanh: string;
    ma_don_vi: string;
    nam_xd: string;
    ds_nam: any;
    trang_thai: any;
    arrBaoCaoQuyNamDeTai: IBaoCaoQuyNamDeTai[];
    currentPage: any;
    maxPage: any;
    findName: any;
    up: any;
...
export class DanhSachBaoCaoQuyNamDeTai extends React.Component<IHienThiDanhSachBaoCaoQuyNamDeTaiProps, IBaoCaoQuyNamDeTaiState> {
    state: IBaoCaoQuyNamDeTaiState = {
    ...
    componentDidMount() {
        // this.layDanhSachBaoCaoQuyNamDeTai();
        ...
        if (!(this.props.match.params.donvi === undefined) && !(this.props.match.params.nam === undefined) &&
            ...
            this.props.getDanhSachBaoCaoQuyNamDeTai(this.props.match.params.donvi, this.props.match.params.nam, this.props.match.params.trangthai);
        }
    ...
    componentWillReceiveProps(nextProps: Readonly<IHienThiDanhSachBaoCaoQuyNamDeTaiProps>, nextContext: any): void {
        const value = [];
        nextProps.ds_bao_cao_quy_nam.map((ele, i) => {
            value.push(ele);
        });
        this.setState({ arrBaoCaoQuyNamDeTai: value,
            maxPage: nextProps.ds_bao_cao_quy_nam.length % 10 > 0 ? parseInt((nextProps.ds_bao_cao_quy_nam.length / 10 + 1).toString(), 10) :
                nextProps.ds_bao_cao_quy_nam.length === 0 ? 1 : parseInt((nextProps.ds_bao_cao_quy_nam.length / 10).toString(), 10)
        });
        if (this.state.maxPage < 1) {
            this.setState({ maxPage: 1 });
        }
        const trang = this.state.currentPage;
        const size = nextProps.ds_bao_cao_quy_nam.length;
        if (size < 10) {
            this.setState({ arrBaoCaoQuyNamDeTai: value });
        } else if ((trang - 1) * 10 > size) {
            this.setState({ arrBaoCaoQuyNamDeTai: nextProps.ds_bao_cao_quy_nam.slice(size - size % 10, size + 1) });
        } else if (trang * 10 > size) {
            this.setState({ arrBaoCaoQuyNamDeTai: nextProps.ds_bao_cao_quy_nam.slice((trang - 1) * 10, size + 1) });
        } else {
            this.setState({ arrBaoCaoQuyNamDeTai: nextProps.ds_bao_cao_quy_nam.slice((trang - 1) * 10, trang * 10) });
        }
    }
    ...
    onClickDenTrang = e => {
        const trang = this.state.currentPage;
        const size = this.props.ds_bao_cao_quy_nam.length;
        if (size < 10) {
            this.setState({ arrBaoCaoQuyNamDeTai: this.state.arrBaoCaoQuyNamDeTai });
        } else if ((trang - 1) * 10 > size) {
            this.setState({ arrBaoCaoQuyNamDeTai: this.props.ds_bao_cao_quy_nam.slice(size - size % 10, size + 1) });
        } else if (trang * 10 > size) {
            this.setState({ arrBaoCaoQuyNamDeTai: this.props.ds_bao_cao_quy_nam.slice((trang - 1) * 10, size + 1) });
        } else {
            this.setState({ arrBaoCaoQuyNamDeTai: this.props.ds_bao_cao_quy_nam.slice((trang - 1) * 10, trang * 10) });
        }
    }

    sortDanhSach = (key, type) => () => {
        let temp = this.state.arrBaoCaoQuyNamDeTai;
        if (key === 1) {
            if (type === 1) {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenDeTai > b.tenDeTai) ? 1 : ((b.tenDeTai > a.tenDeTai) ? -1 : 0));
            } else {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenDeTai < b.tenDeTai) ? 1 : ((b.tenDeTai < a.tenDeTai) ? -1 : 0));
            }
        } else if (key === 2) {
            if (type === 1) {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenLoaiDeTai > b.tenLoaiDeTai) ? 1 : ((b.tenLoaiDeTai > a.tenLoaiDeTai) ? -1 : 0));
            } else {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenLoaiDeTai < b.tenLoaiDeTai) ? 1 : ((b.tenLoaiDeTai < a.tenLoaiDeTai) ? -1 : 0));
            }
        } else if (key === 3) {
            if (type === 1) {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenHangDeTai > b.tenHangDeTai) ? 1 : ((b.tenHangDeTai > a.tenHangDeTai) ? -1 : 0));
            } else {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenHangDeTai < b.tenHangDeTai) ? 1 : ((b.tenHangDeTai < a.tenHangDeTai) ? -1 : 0));
            }
        } else if (key === 4) {
            if (type === 1) {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenChuThe > b.tenChuThe) ? 1 : ((b.tenChuThe > a.tenChuThe) ? -1 : 0));
            } else {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenChuThe < b.tenChuThe) ? 1 : ((b.tenChuThe < a.tenChuThe) ? -1 : 0));
            }
        } else if (key === 5) {
            if (type === 1) {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenNhomDeTai > b.tenNhomDeTai) ? 1 : ((b.tenNhomDeTai > a.tenNhomDeTai) ? -1 : 0));
            } else {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.tenNhomDeTai < b.tenNhomDeTai) ? 1 : ((b.tenNhomDeTai < a.tenNhomDeTai) ? -1 : 0));
            }
        } else if (key === 6) {
            if (type === 1) {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.trangThai > b.trangThai) ? 1 : ((b.trangThai > a.trangThai) ? -1 : 0));
            } else {
                temp = this.state.arrBaoCaoQuyNamDeTai.sort((a, b) => (a.trangThai < b.trangThai) ? 1 : ((b.trangThai < a.trangThai) ? -1 : 0));
            }
        }
        this.setState({ arrBaoCaoQuyNamDeTai: temp, up: type === 0 ? 1 : 0 });
    };
    ...
    onClickFindName = e => {
        this.props.getDanhSachBaoCaoQuyNamDeTaiTheoTen_DonVi_Nam_TrangThai(this.state.findName, this.state.ma_don_vi, this.state.nam_xd, this.state.trang_thai);
    }
    ...
    handleSelectDonVi = (event: React.FormEvent<HTMLSelectElement>) => {
        ...
        if (!(this.state.nam_xd === undefined || this.state.nam_xd === null || this.state.nam_xd === '' || this.state.nam_xd === '-1')) {
            this.layDanhSachBaoCaoQuyNamDeTai(element.value, this.state.nam_xd, this.state.trang_thai);
        }
    }
    ...
    handleSelectTrangThai = (event: React.FormEvent<HTMLSelectElement>) => {
        ...
        if (!(element.value === '-1')) {
            this.layDanhSachBaoCaoQuyNamDeTai(this.state.ma_don_vi, this.state.nam_xd, element.value);
        }
    }
    layDanhSachBaoCaoQuyNamDeTai = (donvi: any, nam: any, trangthai: any) => {
        // const { activePage, itemsPerPage, sort, order } = this.state;
        this.props.getDanhSachBaoCaoQuyNamDeTai(donvi, nam, trangthai);
    };

    render() {
        const { ds_bao_cao_quy_nam, ds_tinh_thanh, ds_don_vi, match, totalItems } = this.props;
        return (
            <div>
                ...
                <Table responsive striped>
                    ...
                    <tbody>
                    {
                        this.state.arrBaoCaoQuyNamDeTai.map((baoCaoQuyNamDeTai, i) => (
                            <tr>
                                <td>{baoCaoQuyNamDeTai.tenDeTai}</td>
                                <td>{baoCaoQuyNamDeTai.tenLoaiDeTai}</td>
                                <td>{baoCaoQuyNamDeTai.tenHangDeTai}</td>
                                <td>{baoCaoQuyNamDeTai.tenChuThe}</td>
                                <td>{baoCaoQuyNamDeTai.tenNhomDeTai}</td>
                                <td style={ { width: '20%' } }>
                                    {
                                        baoCaoQuyNamDeTai.trangThai === 1 ?
                                ...
                                <td className="text-right">
                                    <div className="btn-group flex-btn-group-container">
                                        <Button tag={Link} to={`/quan-ly-nckh/bao-cao-quy-nam-de-tai/${baoCaoQuyNamDeTai.maDeTai}/${this.state.ma_don_vi}/${this.state.nam_xd}/${this.state.trang_thai}/cap-nhat-bien-ban`} color="warning" size="sm">
...
const mapStateToProps = (storeState: IRootState) => ({
    ds_bao_cao_quy_nam: storeState.baocaoquynamdetai.ds_bao_cao_quy_nam_de_tai,
    ds_tinh_thanh: storeState.baocaoquynamdetai.ds_tinh_thanh,
    ds_don_vi: storeState.baocaoquynamdetai.ds_don_vi,
    totalItems: storeState.baocaoquynamdetai.totalItems
});

const mapDispatchToProps = { getDanhSachBaoCaoQuyNamDeTai, getDanhSachBaoCaoQuyNamDeTaiTheoTen_DonVi_Nam_TrangThai,
